# Handoff: clarc script file args — 2026-04-25

## What was accomplished

### `clarc <file>...` now loads files and exits

Previously the `clarc` shell script always dropped into the Arc REPL after
booting. Running `clarc test.arc` had no way to load `test.arc`; the user had
...
The `clarc` script now builds a `(list "f1" "f2" ...)` form for the `:files`
...
- **Pass files as a Lisp list, not a single string.** Leaves room for
  `clarc a.arc b.arc` to load multiple files in order without re-parsing in
